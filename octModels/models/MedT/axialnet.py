# adapted from https://github.com/jeya-maria-jose/Medical-Transformer
# adaptations marked with ########### 
import pdb
import math
import torch
...
########### edited medT version cf source (top)
...
class medt_net(nn.Module):
    ...
    def _forward_impl(self, x):
        ...
        #C: ####edited
        #print(f"forward output of net before adjust: {torch.unique(x), x.shape}")
        #C: ####edited
        ...
        #C: ####edited 
        #print(f"forward output of net: unique vals: {torch.unique(x)}, shape {x.shape}, proba values (softmax)" \
        #    + f" {torch.unique(torch.nn.functional.log_softmax(x))}")
        #C: ####edited
...
########### edited medT for pretraining auto enc cf source (top)
class medt_pretrain_net(nn.Module):

    def __init__(self, block, block_2, layers, zero_init_residual=True,
                 groups=8, width_per_group=64, replace_stride_with_dilation=None,
                 norm_layer=None, s=0.125, img_size = 128,imgchan = 3):
        super(medt_pretrain_net, self).__init__()
        if norm_layer is None:
            norm_layer = nn.BatchNorm2d
        self._norm_layer = norm_layer
        ...
        #=====================================
        # adaptation to pretext task auto enc- applied after self.decoderf  
        # - in_channels 128*s
        # - out_channels 1 -> grayscale image  
        self.pretrain_layer = nn.Conv2d(int(128*s) , 1 , kernel_size=3, stride=1, padding=1)
        #=====================================
    ...
    def _forward_impl(self, x):
        ...
        x = F.relu(self.pretrain_layer(x))
...
########### edited medT for pretraining auto enc
...
def MedT_pretrain(pretrained=False, **kwargs):
    model = medt_pretrain_net(AxialBlock_dynamic,AxialBlock_wopos, [1, 2, 4, 1], s= 0.125,  **kwargs)
    return model
